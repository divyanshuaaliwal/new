...
export const Hotels = () => {
    ...
    useEffect( () => {
        setFilteredHotels(allUjjainHotels);
        setCurrentPage(1);
    }, []);

    const starParam = searchParams.get("star");
    useEffect(() => {
        const starArray = starParam ? starParam.split(',').map(Number) : [];
        setSelectedStars(starArray);
    }, [starParam]);
    ...
    useEffect(() => {
        ...
        let updatedHotels = [...(allUjjainHotels || [])];
                                ...
                                currentHotels.map((hotel, index) => (
                                    <div key={index} className={styles.hotelCard}>
                                        <div className={styles.hotelGrid}>
                                            <Link
                                                href={`/hotels-in-ujjain/${hotel.slug}`}
